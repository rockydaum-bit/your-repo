...
class FakeYTA:
    ...
    def query_channel_kpis(self, *, start: date, end: date):
        return FakeChannelKpis(
            views=1000,
            watch_time_hours=50.0,
            avg_view_duration_sec=180.0,
            subs_net=10,
            estimated_revenue_usd=120.0,
        )
    ...
    def query_videos_kpis(self, *, start: date, end: date, youtube_video_ids: list):
        return [
            FakeVideoKpis(
                ...
                avg_view_duration_sec=120.0,
                estimated_revenue_usd=12.0,
            )
...
def test_run_daily_rollup_creates_rollup_and_inserts_db(tmp_path, monkeypatch):
    ...
    (prompts_dir / "schemas" / "analytics_rollup.schema.json").write_text(
        json.dumps(analytics_schema)
    )
    ...
    with connect(paths.db_path) as con:
        con.execute(
            "INSERT INTO videos (channel_id, video_id, run_id, topic, status, youtube_video_id) VALUES (?, ?, ?, ?, ?, ?)",
