...
def test_generate_upload_metadata_writes_valid_metadata(tmp_path, monkeypatch):
    ...
    cfg = EngineConfig(
        engine_root=engine_root, openai_api_key="test", openai_model="gpt-4o"
    )
    ...
    (prompts_dir / "system" / "publishing_seo_system.txt").write_text(
        "You are the Publishing & SEO Agent. Output JSON only."
    )
    ...
    agent.generate_upload_metadata(
        channel_id="channel_001_ai_tools",
        run_id="r1",
