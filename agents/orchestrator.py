...
class Orchestrator:
    ...
    def run_weekly_growth_loop(self) -> None:
        ...
        try:
            ...
            try:
                if summary_path and os.path.exists(summary_path):
                    ...
                    updated_at = (
                        rs.get("finished_at") or rs.get("started_at") or updated_at
                    )
        ...
        log_event(
            ...
            self._ts(),
            "INFO",
            "weekly_growth_loop_complete",
            json.dumps(artifacts),
        )

        # Weekly optimization autopatch (requires rollups)
        try:
            opt_path = self.optimize.run_weekly_optimization(
                channel_id="channel_001_ai_tools"
            )
    ...
    def _one_video_pipeline(self, channel_id: str, run_id: str) -> dict[str, str]:
        ...
        """
        base = os.path.join(
            self.cfg.engine_root, "assets", "channels", channel_id, "pipeline"
        )
        ...
        if os.path.exists(run_summary_path):
            try:
                ...
                try:
                    schema = (
                        load_json(schema_path) if os.path.exists(schema_path) else None
                    )
                    if schema is not None:
                        validate_json_against_schema(summary, schema)
                except SchemaValidationError as sve:
                    self.logger.warning(
                        f"Existing run summary failed schema validation: {sve}; archiving and starting fresh."
                    )
                    bad_archive = (
                        run_summary_path
                        + f".invalid.{datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}"
                    )
                    try:
                    ...
                    self.logger.warning(
                        f"Error validating existing summary: {e}; starting fresh."
                    )
                    summary = {}
            except Exception:
                summary = {}
        ...
        try:
            ...
            summary["errors"].append(
                {"step": "plan", "message": str(e), "ts": self._ts()}
            )
        ...
        if not plan_ok:
            ...
            summary["errors"].append(
                {
                    "step": "script",
                    "message": "blocked_by_failed_dependency",
                    "ts": self._ts(),
                }
            )
        else:
            try:
                ...
                summary["errors"].append(
                    {"step": "script", "message": str(e), "ts": self._ts()}
                )
        ...
        if not script_ok:
            ...
            try:
                ...
                summary["errors"].append(
                    {"step": "tts", "message": str(e), "ts": self._ts()}
                )
        ...
        if not script_ok:
            ...
            try:
                ...
                summary["errors"].append(
                    {"step": "storyboard", "message": str(e), "ts": self._ts()}
                )
        ...
        if not script_ok:
            ...
            try:
                if _should_skip(meta_path):
                    ...
                    self.pub.generate_upload_metadata(
                        channel_id=channel_id,
                        run_id=run_id,
                        script_path=script_path,
                ...
                summary["errors"].append(
                    {"step": "metadata", "message": str(e), "ts": self._ts()}
                )
