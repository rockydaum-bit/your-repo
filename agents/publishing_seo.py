...
from utils.db import upsert_video_publish, upsert_video_publication, insert_orchestrator_event
        ...
        *,
        ...
        if self.youtube_client_factory:
            ...
            try:
                ...
                try:
                    yt = YouTubeClient(self.cfg.youtube.client_secrets_path, self.cfg.youtube.token_path)
                except TypeError:
                    try:
        ...
        while attempt < 5:
            try:
                privacy_status = getattr(self.cfg.youtube, "privacy_status_default", "private")
                ...
                if thumbnail_path:
                    try:
                        yt.set_thumbnail(youtube_video_id=res.youtube_video_id, thumbnail_path=thumbnail_path)
                    except Exception:
                        # thumbnail is non-critical; log and continue
                        pass

                # Persist to DB (idempotent) - legacy and new publications table
                video_id = meta.get("video_id") or os.path.splitext(os.path.basename(final_video_path))[0]
                ...
                try:
                    upsert_video_publication(
                        ...
                        published_ts,
                        status="published",
                        metadata={"thumbnail_path": thumbnail_path} if thumbnail_path else None,
                    )
                except Exception:
                ...
                insert_orchestrator_event(self.paths.db_path, published_ts, "INFO", "video_published", payload)
                ...
                insert_orchestrator_event(self.paths.db_path, datetime.utcnow().isoformat(timespec="seconds") + "Z", "WARN", "video_publish_retry", {"attempt": attempt, "error": str(e)})
                time.sleep(wait)
        ...
        try:
            video_id = meta.get("video_id") or os.path.splitext(os.path.basename(final_video_path))[0]
            upsert_video_publish(self.paths.db_path, channel_id, video_id, run_id, "", failed_ts, status="upload_failed")
            try:
                upsert_video_publication(
                    self.paths.db_path,
                    channel_id,
        ...
        insert_orchestrator_event(self.paths.db_path, failed_ts, "ERROR", "video_publish_failed", {"run_id": run_id, "error": str(last_err)})
        raise RuntimeError(f"Upload failed after retries: {last_err}")

    def generate_upload_metadata(self, channel_id: str, run_id: str, script_path: str, output_path: str) -> None:
